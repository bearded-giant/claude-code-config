...
description: Use this agent when processing, analyzing, or diagnosing data pipelines and analytics workflows. This includes investigating data output deltas, debugging metric calculations, designing multi-step aggregation pipelines, implementing data transformations, analyzing Snowflake queries, optimizing database operations, or wrapping analytics processes in Flask APIs. The agent excels at Python-heavy data processing tasks and can handle file processing workflows, ETL operations, and data quality investigations.\n\nExamples:\n<example>\nContext: User needs to investigate why metrics are showing unexpected changes\nuser: "Why are our daily revenue metrics 15% lower than yesterday?"\nassistant: "I'll use the data-analytics-processor agent to investigate this data delta"\n<commentary>\nSince this involves diagnosing data output deltas and metric calculations, use the data-analytics-processor agent.\n</commentary>\n</example>\n<example>\nContext: User needs to create a new aggregation pipeline\nuser: "Create a pipeline that aggregates user activity data by region and product"\nassistant: "I'll use the data-analytics-processor agent to design this multi-step aggregation pipeline"\n<commentary>\nThis requires designing a multi-step data aggregation pipeline, which is a core capability of the data-analytics-processor agent.\n</commentary>\n</example>\n<example>\nContext: User needs to wrap analytics logic in an API\nuser: "Take this metric calculation and expose it as a Flask endpoint"\nassistant: "I'll use the data-analytics-processor agent to properly wrap this analytics process in a Flask API"\n<commentary>\nThe agent specializes in wrapping analytics processes in Flask APIs.\n</commentary>\n</example>
model: opus
color: blue
---
