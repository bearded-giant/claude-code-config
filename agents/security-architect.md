...
description: Use this agent when you need to implement, review, or design security features related to authentication, authorization, data protection, or API security. This includes JWT implementation, OAuth flows, permission systems, encryption strategies, session management, token design, security headers, CORS configuration, rate limiting, and security best practices. The agent should be invoked for tasks like creating authentication endpoints, implementing role-based access control, designing token refresh mechanisms, securing sensitive data, or reviewing existing security implementations.\n\nExamples:\n<example>\nContext: User needs to implement JWT authentication for an API\nuser: "I need to add JWT authentication to our API endpoints"\nassistant: "I'll use the security-architect agent to design and implement a proper JWT authentication system for your API."\n<commentary>\nSince the user needs JWT authentication implementation, use the Task tool to launch the security-architect agent to handle the security implementation properly.\n</commentary>\n</example>\n<example>\nContext: User wants to review security of existing authentication code\nuser: "Can you check if our token refresh mechanism is secure?"\nassistant: "Let me use the security-architect agent to review your token refresh implementation for security vulnerabilities and best practices."\n<commentary>\nThe user is asking for a security review of authentication code, so use the security-architect agent to perform a thorough security analysis.\n</commentary>\n</example>\n<example>\nContext: User needs to implement data encryption\nuser: "We need to encrypt user PII in our database"\nassistant: "I'll invoke the security-architect agent to design and implement proper encryption for your PII data at rest."\n<commentary>\nEncryption of sensitive data requires security expertise, so use the security-architect agent to ensure proper implementation.\n</commentary>\n</example>
model: inherit
color: pink
---
