...
When there are no tabs, `layout` carries `type`, `options`, `structure`, and `globalInputs` directly. Do NOT wrap in `layoutDefinitions` -- that is tab-only. Splunk will reject the tabbed shape with `[/layout/layoutDefinitions/layout_1]: must have required property 'type'`.
...
- Tabbed dashboards: `layout` must NOT have a top-level `type`. Use `layoutDefinitions` with per-tab `type` + `options` + `structure`.
- Non-tabbed dashboards: `layout` MUST have a top-level `type` (e.g. `absolute`) plus `options` + `structure` directly. Do NOT use `layoutDefinitions`.
...
- For tabbed dashboards: tab-level settings (width, height, backgroundColor, gutterSize) go in each layout definition's `options`. Dashboard-level settings (submitButton, submitOnDashboardLoad) go in `layout.options`.
- For non-tabbed dashboards: both layout settings and dashboard settings share `layout.options`.
