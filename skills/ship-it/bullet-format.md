...
Consumed by `ship-it` Step 3 for the personal bullet path (`brief`/`short`/`--brief`,
and the GitHub default). Base-branch resolution, routing (active feature dir →
`.giantmem/` → repo root), write, and print are owned by ship-it Step 3 — this file
owns the FORMAT only.

## Building the description

1. **Branch context**: current branch, commits via `git log <base>..HEAD`, full diff via `git diff <base>..HEAD`. Read diff for understanding.

2. **Identify the theme, not the changes**: read the diff to understand what the MR *accomplishes* — the user-visible behavior change, the architectural shift, the bug fix. Write to that level. Reviewers read the diff for what changed file-by-file; the description tells them *why this MR exists* and *how the pieces fit together at a high level*.
...
3. **Betaflags (diff only, NOT whole codebase)**: scan added lines for `BetaService` imports or `is_enabled(` calls. Note flag strings. Omit section if none.

4. **API endpoints**: find new/modified Flask routes. Generate curl examples using `api.rechargeapps.com`. Include example response body when inferable.
